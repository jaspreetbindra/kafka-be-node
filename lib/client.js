...
if(global.beOverrideLogger){
  global.beOverrideLogger = console
}
...
Client.prototype.sendFetchRequest = function (consumer, payloads, fetchMaxWaitMs, fetchMinBytes, maxTickMessages) {
  ...
  var decoder = protocol.decodeFetchResponse(function (err, type, message) {
    let beDebugDataShouldBeLogged = false
    let beDebugDataDetailsMessage = {}
    if (type === "message"
      && message
      && message instanceof Object
      && Object.keys(message).length > 1) {
      beDebugDataShouldBeLogged = true;
      beDebugDataDetailsMessage = JSON.stringify({
        parition : message.partition,
        highWaterMark : message.highWaterOffset,
      })
    }
    else if (
      type === "done"
      ...
      && message[Object.keys(message)[0]] instanceof Object
      && Object.keys(message[Object.keys(message)[0]]).length
    ) {
      beDebugDataShouldBeLogged = true;
      beDebugDataDetailsMessage = JSON.stringify(message[Object.keys(message)[0]])
    }
  if(beDebugDataShouldBeLogged ) beOverrideLogger.log("Received request to decode for type : " + type + " and details :  " +  beDebugDataDetailsMessage);
    if (err) {
    ...
    if (type === 'message') {
      ...
      beDebugDataShouldBeLogged && beOverrideLogger.log("Raising message event on event-name `message` " + " and details " + beDebugDataDetailsMessage);
      consumer.emit('message', message);
    } else {
      beDebugDataShouldBeLogged && beOverrideLogger.log("AUTO COMMITING STARTING NOW - 2 -> EVENT RAISED"  + " and details " + beDebugDataDetailsMessage)
...
Client.prototype.sendOffsetCommitV2Request = function (group, generationId, memberId, payloads, cb) {
  beOverrideLogger.log("AUTO COMMITING SEDNING REQUEST NOW")
